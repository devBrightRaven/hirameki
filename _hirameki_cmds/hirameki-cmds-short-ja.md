...
3 つのモード：
- **`status`** — 即時スナップショット：コンテンツテーマ・最近の活動・統計。セッション開始時に実行するのに最適。
- **`status week`** — 週次レビュー：宣言した優先事項と実際のファイル変更を比較してギャップを発見。
- **`status patterns`** — 潜流とクラスター：独立した記事のない繰り返し登場するテーマと、形成中のアイデアグループを表面化。
...
### `/hirameki:tidy [fix]`
プロパティの健全性チェック。欠落フィールド・タグの不整合・冗長性・統合機会を報告。健全性スコアを出力。
`fix` を追加で確実なものを自動修正、判断が必要なものは個別確認。
書き込み先：`{analysis}/tidy/`
