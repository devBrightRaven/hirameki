...
**第二階段 — 交叉引用：** 掃描內容資料夾和近期 daily notes，找出觸及相同主張或概念的 vault 筆記。每筆：`[[檔名]]` + 該 vault 筆記與此文章的關係（支持 / 矛盾 / 延伸 / 被延伸）。
...
**用途：** 進度快照，追加到 daily note。
**輸入：** 可選重點描述。
**需要的資料夾：** daily、templates。

掃描本次 session 的檔案操作（建立、修改、刪除）和今天的 daily note。
...
搜尋 vault 中與問題相關的筆記。優先：直接探討此主題的內容資料夾筆記、相關的永久概念卡片、作者表達立場的 journal 條目、有相關觀察的 daily notes。上限：5 個最相關來源。
...
掃描：所有內容資料夾（深度 2 層）、daily notes 最近 7 天。
...
讀取：所有內容資料夾最近 7 天的修改紀錄、最近 7 天的 daily notes。
...
如果 daily notes 不足 3 天，標注「紀錄不足，落差分析可能不準確」。
...
**用途：** 從既有內容中收割可行動的想法。
**輸入：** 可選 `save` 寫入摘要。
**需要的資料夾：** research、daily notes、inbox + 所有內容資料夾。
...
**掃描範圍：** 所有內容資料夾（遞迴）、所有 inbox 檔案、daily notes（最近 30 天）。
