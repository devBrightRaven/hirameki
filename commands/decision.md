...
Read `vault:` from `~/.claude/vault-local.md` for the vault root, then read `## Vault Structure` from `<vault>/AGENTS.md` (fall back to `~/.claude/vault-local.md`, then `~/.claude/CLAUDE.md` for setups predating 1.4.3) to get the journal folder and language. If setup is incomplete, stop and respond: "Setup not complete. Please run `/hirameki:__init` first."
