---
description: BRW 系統共用的 vault 偵測與資料夾解析邏輯（供其他 commands 引用，不單獨執行）
---
...
之後所有 brw command 的輸出（終端顯示和檔案寫入）都使用這個語言。
...
- **analysis**：`_brw_analysis/`, `_agent_analysis/`, `_claude_code_analysis/`, `Analysis/`, `_analysis/`, `analysis/`
- **logs**：`_brw_logs/`, `_agent_logs/`, `_claude_code_feedback/`, `Logs/`, `_logs/`, `logs/`
...
下次執行任何 brw command 時，如果 vault 的 CLAUDE.md 裡已有 `## Vault Structure`，直接讀取，不再偵測。
...
**BRW 系統資料夾（記錄在 `## Vault Structure` 中的）：**
- daily-notes 對應的資料夾
- inbox 對應的資料夾
- analysis 對應的資料夾
- logs 對應的資料夾
...
排除以上之後，剩下的就是使用者的內容資料夾。例如使用者建了 `Writing/`、`Projects/`、`Research/`、`Notes/`，這些全部會被 brw-status、brw-weekplan、brw-harvest、brw-as-me、brw-promote 等 command 掃描。
