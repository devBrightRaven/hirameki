...
__init 負責首次設定 Hirameki 環境。設定完成後，結果寫入 vault 根目錄的 `CLAUDE.md`，日常 commands 直接讀取，不再呼叫 __init。

## 日常 Commands 的讀取方式

所有其他 hirameki commands 在執行前：

1. 讀取 vault 根目錄的 `CLAUDE.md` 中的 `## Vault Structure` 段落
...
觸發條件：vault 根目錄的 `CLAUDE.md` 不存在 `## Vault Structure`，或使用者主動執行 `/hirameki:__init` 且現有設定為空。
...
將以下內容寫入 vault 根目錄的 `CLAUDE.md`（不存在則建立）：
